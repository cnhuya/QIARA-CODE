...
// --- Permissionless Asset Listing ---
    ...
    public entry fun p_allow_sub_owner(name: String, sub_owner: address, ctx: &mut TxContext) {
        let sender = tx_context::sender(ctx);
        let data = vector[
            Event::create_data_struct(string::utf8(b"user"), string::utf8(b"address"), bcs::to_bytes(&sender)),
            Event::create_data_struct(string::utf8(b"sub_owner"), string::utf8(b"address"), bcs::to_bytes(&sub_owner)),
            Event::create_data_struct(string::utf8(b"name"), string::utf8(b"string"), bcs::to_bytes(&name)),
        ];
    ...
    public entry fun p_remove_sub_owner(name: String, sub_owner: address, ctx: &mut TxContext) {
        let sender = tx_context::sender(ctx);
        let data = vector[
            Event::create_data_struct(string::utf8(b"user"), string::utf8(b"address"), bcs::to_bytes(&sender)),
            Event::create_data_struct(string::utf8(b"sub_owner"), string::utf8(b"address"), bcs::to_bytes(&sub_owner)),
