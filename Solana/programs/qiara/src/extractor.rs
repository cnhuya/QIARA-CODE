...
pub const E_INVALID_INPUT_LENGTH: u32 = 400;
...
/// Extract contiguous 32-byte chunk
...
/// Convert 32-byte Little-Endian chunk directly to a u128
pub fn bytes_to_u128_le(bytes: &[u8; 32], offset: usize) -> u128 {
    let mut temp = [0u8; 16];
    temp.copy_from_slice(&bytes[offset..offset + 16]);
    u128::from_le_bytes(temp)
}

/// Reconstructs VariableHeader and VariableName as Strings, and VariableData as Vec<u8>
pub fn extract_variable_strings(inputs: &[u8]) -> Result<(String, String, Vec<u8>)> {
    if inputs.len() < 224 {
        return err!(crate::QiaraError::InvalidInputLength);
    }

    // 1. Unpack VariableHeader (Upper bits of index 2)
    let chunk_2 = extract_chunk(inputs, 2)?;
    let mut header_bytes = chunk_2[2..].to_vec(); // Skip first 2 bytes (shift right 16 bits equivalent)
    header_bytes.retain(|&x| x != 0);
    let variable_header = String::from_utf8(header_bytes).unwrap_or_default();

    // 2. Reconstruct VariableName from Index 3 and 4
    ...
    variable_data.extend_from_slice(&chunk_6[16..32]); // High 16 bytes
    variable_data.extend_from_slice(&chunk_5[16..32]); // Low 16 bytes

    Ok((variable_header, variable_name, variable_data))
}

/// Extract uncompressed SEC1 validator public key (65 bytes, prefix 0x04)
pub fn extract_validator_pubkey(inputs: &[u8]) -> Result<Vec<u8>> {
    ...
    let mut pubkey = vec![0x04]; // SEC1 uncompressed prefix
...
/// Extract removal status
pub fn extract_validator_is_removal(inputs: &[u8]) -> Result<bool> {
    let chunk_2 = extract_chunk(inputs, 2)?;
    // Shift right by 16 bits and mask LSB: check if 17th bit (byte index 2, bit 0) is set
    Ok((chunk_2[2] & 1) == 1)
}

/// Extract balance transaction data (Bits mapped directly to bytes)
...
pub fn extract_user_address(inputs: &[u8]) -> Result<Pubkey> {
    ...
    let low = extract_chunk(inputs, 2)?; // INDEX_USER_L
    let high = extract_chunk(inputs, 3)?; // INDEX_USER_H
...
pub fn extract_provider(inputs: &[u8]) -> Result<String> {
    ...
    let chunk_4 = extract_chunk(inputs, 4)?; // INDEX_VAULT
    let mut provider_bytes = chunk_4.to_vec();
    provider_bytes.retain(|&x| x != 0);
    Ok(String::from_utf8(provider_bytes).unwrap_or_default())
}

pub fn extract_root(inputs: &[u8], index: usize) -> Result<[u8; 32]> {
    extract_chunk(inputs, index)
}

/// Replicates keccak256(abi.encodePacked(input[0..5]))
pub fn build_nullifier(inputs: &[u8]) -> Result<[u8; 32]> {
    ...
    for i in 0..6 {
        ...
        chunk_be.reverse(); // Convert Little Endian proof inputs to Big Endian (Solidity format)
