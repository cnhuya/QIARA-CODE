module dev::QiaraTokenTypesV67 {
    ...
    const TOKEN_PREFIX: vector<u8> = b"Qiara141 ";
...
// === INIT === //
    ...
    fun x_init(signer: &signer) acquires Tokens {
        ...
        // 1. QIARA TOKEN
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Qiara"), utf8(b"Qiara"), 
            vector[
        ...
        // 2. SOLANA (SOL)
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Solana"), utf8(b"Solana"), 
        ...
        // 4. USDG
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 USDG"), utf8(b"USDG"), 
            vector[
        ...
        // 5. JLP
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 JLP"), utf8(b"JLP"), 
        ...
        // 6. BURNED QIARA
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Burned Qiara"), utf8(b"Burned Qiara"), 
            vector[utf8(b"0x0")], 
            vector[utf8(b"Aptos")],
            vector[8u8]
        );
        ...
        // 7. USDC
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 USDC"), utf8(b"USDC"), 
            vector[
        ...
        // 8. USDT
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 USDT"), utf8(b"USDT"), 
            vector[
        ...
        register_token_with_chains(signer, utf8(b"Qiara141 AUSD"), utf8(b"AUSD"), 
            vector[
        ...
        register_token_with_chains(signer, utf8(b"Qiara141 earnAUSD"), utf8(b"earnAUSD"), 
            vector[
        ...
        // 10. ETHEREUM (ETH)
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Ethereum"), utf8(b"Ethereum"), 
            vector[
        ...
        // 11. BITCOIN (BTC)
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Bitcoin"), utf8(b"Bitcoin"),
            vector[
        ...
        // 12. MONAD
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Monad"), utf8(b"Monad"), 
            vector[
        ...
        // 13. APTOS
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Aptos"), utf8(b"Aptos"), 
            vector[utf8(b"0x0")], 
            vector[utf8(b"Aptos")],
            vector[8u8]
        );
        ...
        // 14. SUI & DEEPBOOK
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Sui"), utf8(b"Sui"), 
            vector[
        ...
        register_token_with_chains(signer, utf8(b"Qiara141 Deepbook"), utf8(b"Deepbook"), 
            vector[
        ...
        // 15. VIRTUALS
        // -------------------------------------------------------------
        register_token_with_chains(signer, utf8(b"Qiara141 Virtuals"), utf8(b"Virtuals"), 
            vector[
...
// === VIEW & LOOKUP FUNCTIONS === //
    ...
    public entry fun add_token_chain(signer: &signer, token: String, nick_name: String, token_address: String, chain: String,decimals: u8) acquires Tokens {
        let tokens = borrow_global_mut<Tokens>(@dev);
    ...
    public entry fun remove_token_chain(_signer: &signer,token: String,chain: String) acquires Tokens {
        let tokens = borrow_global_mut<Tokens>(@dev);

        if (!map::contains_key(&tokens.map, &token)) return;
