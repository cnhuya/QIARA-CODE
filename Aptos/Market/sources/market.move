module dev::QiaraVaultsV82 {
    ...
    use dev::QiaraTokensCoreV58::{Self as TokensCore, CoinMetadata, Access as TokensCoreAccess};
    use dev::QiaraTokensOmnichainV58::{Self as TokensOmnichain};
    use dev::QiaraTokensMetadataV58::{Self as TokensMetadata, VMetadata, Access as TokensMetadataAccess};
    use dev::QiaraTokensTiersV58::{Self as TokensTiers};
    use dev::QiaraWrapperGateV58::{Self as WrapperGate};

    use dev::QiaraMarginV60::{Self as Margin, Access as MarginAccess};
    use dev::QiaraRanksV60::{Self as Points, Access as PointsAccess};
    use dev::QiaraRIV60::{Self as RI};
    use dev::QiaraBurnedQiaraV60::{Self as BurnedQiara};

    use dev::QiaraTokenTypesV58::{Self as TokensTypes};
    ...
    use dev::QiaraLiquidityV78::{Self as Liquidity, Access as LiquidityAccess};
    use dev::QiaraTokenVaultsV78::{Self as TokenVaults, Access as TokenVaultsAccess};
...
// === CONSENSUS INTERFACE === //
  ...
  public fun c_bridge_withdraw(
    ...
    public fun c_bridge_borrow(validator: &signer, shared: String, sender: vector<u8>,token: String, chain: String, provider: String, amount: u64, permission: Permission) acquires Permissions {
        let amount_u256 = (amount as u256)*1000000000000000000;
...
// === NATIVE INTERFACE === //
    ...
    public entry fun unstake(signer: &signer, shared: String, token: String, chain: String, provider: String, amount: u64) acquires Permissions {
        let sender = bcs::to_bytes(&signer::address_of(signer));
        let amount_u256 = (amount as u256) * 1000000000000000000;
        ...
        let data = vector[
            Event::create_data_struct(utf8(b"sender"), utf8(b"address"), sender),
            Event::create_data_struct(utf8(b"shared"), utf8(b"string"), bcs::to_bytes(&shared)),
            Event::create_data_struct(utf8(b"token"), utf8(b"string"), bcs::to_bytes(&token)),
            Event::create_data_struct(utf8(b"chain"), utf8(b"string"), bcs::to_bytes(&chain)),
            Event::create_data_struct(utf8(b"provider"), utf8(b"string"), bcs::to_bytes(&provider)),
            Event::create_data_struct(utf8(b"amount"), utf8(b"u256"), bcs::to_bytes(&amount_u256)),
            Event::create_data_struct(utf8(b"fee_refund"), utf8(b"u256"), bcs::to_bytes(&fee_refund)),
        ];
    ...
    public entry fun withdraw(signer: &signer, shared: String, token: String, chain: String, provider: String, amount: u64) acquires Permissions {
        ...
        // Accrue after
        let (total_rewards, total_interest, user_borrow_interest, user_lend_rewards, user_points, total_apr, borrow_apr, utilization, price, user_gas_reducted, user_xp_increased, shares_ratio) = new_accrue(signer, shared, sender_bytes, token, chain, provider);
    ...
    public entry fun borrow(signer: &signer, shared: String, token: String, chain: String, provider: String, amount: u64) acquires Permissions {
        ...
        Liquidity::borrow_token(signer, shared, token, chain, provider, amount_u256_taxed, Liquidity::give_permission(&borrow_global<Permissions>(@dev).liquidity));
...
// === HELPERS === //
   ...
   public fun new_accrue(signer: &signer,shared: String, user: vector<u8>, token: String, chain: String, provider: String): (u256,u256, u256, u256, u256, u256, u256, u256, u256, u256,u256,u256) acquires Permissions {
